...
function DashboardPage() {
  ...
  return (
    ...
    >
      <div
        className="mx-auto flex w-full max-w-[1440px] flex-col px-6 pb-8 pt-8 sm:px-8 lg:px-12 lg:pb-3 lg:pt-3 xl:px-14"
        data-testid="industrialization-dashboard-main"
      >
        <DashboardHeader />

        <section className="mt-3 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(405px,0.9fr)] lg:items-stretch xl:grid-cols-[minmax(0,1fr)_minmax(470px,0.96fr)]">
        ...
        <section className="mt-3 border-b border-[rgba(217,222,229,0.9)]">
        ...
        <section className="mt-4 overflow-hidden rounded-[14px] border border-[var(--bocar-border)] bg-white shadow-[0_12px_28px_rgba(0,46,93,0.06)]">
              ...
              <thead>
                <tr className="bg-[#eef1f5]">
                  {['ID', 'MATERIAL', 'CREADO POR', 'FECHA', 'ACCION'].map((header) => (
                    <th
                      key={header}
                      className="border-b border-[var(--bocar-border)] px-5 py-4 text-left text-[13px] font-medium text-[var(--bocar-text)] lg:px-4 lg:py-2"
                    >
                      {header}
                    </th>
                  ))}
                ...
                {hasRows ? (
                  visibleRows.map((row) => (
                    <tr key={row.id} className="transition hover:bg-[rgba(245,247,250,0.84)]">
                      <td className="border-b border-[rgba(217,222,229,0.72)] px-5 py-4 text-[13px] text-[var(--bocar-blue-70)] lg:px-4 lg:py-2">
                        {row.id}
                      </td>
                      <td className="border-b border-[rgba(217,222,229,0.72)] px-5 py-4 text-[13px] lg:px-4 lg:py-2">
                        {row.material}
                      </td>
                      <td className="border-b border-[rgba(217,222,229,0.72)] px-5 py-4 text-[13px] lg:px-4 lg:py-2">
                        {row.createdBy}
                      ...
                      <td className="border-b border-[rgba(217,222,229,0.72)] px-5 py-4 lg:px-4 lg:py-2">
                        <button
                          type="button"
