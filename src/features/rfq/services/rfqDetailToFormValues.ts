...
export function mapDetailToMoldFormValues(dto: RfqDetailDto): MoldFormValues {
  ...
  return {
    ...
    part_name: text(raw, 'part_name') || text(raw, 'PT') || text(raw, 'DESC'),
    ...
    three_plate: text(raw, 'three_plate_mold') || text(raw, 'ThirdPSupp'),
    tt: text(raw, 'TT'),
...
export function mapDetailToTrimmingFormValues(dto: RfqDetailDto): TrimmingFormValues {
  const raw = dto as RawRfq;
  const otherInfo = {
    frame_refur: consideration(raw, 'oi_frame_refurbishment'),
    elec_wires: consideration(raw, 'oi_set_of_electric_wires'),
    others: { checked: yesNo(raw, 'oi_others_applies'), notes: text(raw, 'oi_others') },
    delivery_date: {
      checked: yesNo(raw, 'oi_delivery_date_imex_applies'),
      notes: dateText(raw, 'oi_delivery_date_imex'),
    },
    ejector_fixed: {
      checked: yesNo(raw, 'oi_ejector_fixed_applies'),
      notes: text(raw, 'oi_ejector_fixed_note'),
    },
  };
  ...
  return {
    ...
    previous_job: text(raw, 'previous_job'),
    ...
    punch_pins_required: yesNo(raw, 'punch_pins_required'),
    ...
    pg_part_dimension: text(raw, 'part_dimension'),
    ...
    ts_intro_extraction: text(raw, 'introduction_extraction_process'),
    ts_biscuit_position: text(raw, 'biscuit_position'),
    ts_qty_punch_pins: text(raw, 'quantity_of_punch_pins'),
    ts_temp_trimmed: text(raw, 'temperature_when_trimmed'),
    ts_ejector_fixed_side: text(raw, 'oi_ejector_system_fixed_side'),
    comments: text(raw, 'comments'),
    files: mapUploadedFiles(dto),
  };
}
